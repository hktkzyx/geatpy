...
class C2_DTLZ2(ea.Problem):  # 继承Problem父类
    ...
    def evalVars(self, Vars):  # 目标函数
        ...
        f = np.hstack([
            np.fliplr(np.cumprod(np.cos(Vars[:, :self.M - 1] * np.pi / 2), 1)),
            ones_metrix
        ]) * np.hstack([
            ones_metrix,
            np.sin(Vars[:, range(self.M - 2, -1, -1)] * np.pi / 2)
        ]) * (1 + g)
        # 计算违反约束程度矩阵的值
        CV = np.min([
            np.min(
                (f - 1)**2 + np.sum(f**2, 1, keepdims=True) - f**2 - self.r**2,
                1,
                keepdims=True),
            np.sum((f - 1 / np.sqrt(self.M))**2, 1, keepdims=True) - self.r**2
        ],
                    0)
        return f, CV
